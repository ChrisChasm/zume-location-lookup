...
/**
 * Location_Lookup_IP_Tab
 *
 * @class Location_Lookup_IP_Tab
 * @version	0.1
 * @since 0.1
 * @package	Location_Lookup_IP_Tab
...
class Location_Lookup_IP_Tab {
    ...
    public function ipinfo_page () {
        ...
        if (get_option('_db_option_type') == 'WPDB') {
            ...
            $coordinates = Location_Lookup_Coordinates_KML::get_tract_kml_coordinates ($census_result['geoid'], $state ); // return coordinates from KML files
        }

        Location_Lookup_Map::get_map($census_result['zoom'], $google_result['lng'], $google_result['lat'], $coordinates); // return maps
    }
